...
/***  EDIT S_CODE_COMBINE.JS IF ANY EDITS MADE TO THIS FILE  ***/
/***  EDIT S_CODE_COMBINE.JS IF ANY EDITS MADE TO THIS FILE  ***/
/***  EDIT S_CODE_COMBINE.JS IF ANY EDITS MADE TO THIS FILE  ***/
/***  EDIT S_CODE_COMBINE.JS IF ANY EDITS MADE TO THIS FILE  ***/
/***  EDIT S_CODE_COMBINE.JS IF ANY EDITS MADE TO THIS FILE  ***/
...
if(getCurrSite()=='Pulte')
    s_account="<%= ConfigurationManager.AppSettings["PulteSiteCatalystAcct"] %>"; //phPulte       ACCOUNT NUMBER(S)
else if(getCurrSite()=='DelWebb')
    s_account="<%= ConfigurationManager.AppSettings["DelWebbSiteCatalystAcct"] %>"; //phDelWebb   ACCOUNT NUMBER(S)
else if(getCurrSite()=='Centex')
    s_account="<%= ConfigurationManager.AppSettings["CentexSiteCatalystAcct"] %>"; //phCentex     ACCOUNT NUMBER(S)
else if(getCurrSite()=='DiVosta')
    s_account="<%= ConfigurationManager.AppSettings["DiVostaSiteCatalystAcct"] %>"; //phDiVosta   ACCOUNT NUMBER(S)
...
var s;
if (s_account.toLowerCase() == "phdev")
    s = s_gi(s_account);
else
    s = s_gi(s_account + ",phGlobal");
...
s.linkInternalFilters = "javascript:,pulte.com"
s.linkLeaveQueryString = false

//s.linkTrackVars = "eVar1,eVar3,eVar6,eVar9,eVar11,eVar12,eVar13,eVar14,eVar15,eVar16,eVar17,eVar18,eVar19,eVar20,eVar21,eVar22,eVar23,eVar26,eVar27,eVar28,eVar29,eVar31,eVar34,eVar35,eVar36,eVar37,eVar38,eVar39,eVar40,eVar41,eVar42,events";
s.linkTrackVars = "eVar1,eVar2,eVar3,eVar4,eVar5,eVar6,eVar7,eVar8,eVar9,eVar10,eVar11,eVar12,eVar13,eVar14,eVar15,eVar16,eVar17,eVar18,eVar19,eVar20,eVar21,eVar22,eVar23,eVar24,eVar25,eVar26,eVar27,eVar28,eVar29,eVar30,eVar31,eVar32,eVar33,eVar34,eVar35,eVar36,eVar37,eVar38,eVar39,eVar40,eVar41,eVar42,eVar43,eVar44,eVar45,eVar46,eVar47,eVar48,eVar49,eVar50,eVar51,eVar52,eVar53,eVar54,eVar55,eVar56,eVar57,eVar58,eVar59,eVar60,eVar61,eVar62,eVar63,eVar64,eVar65,eVar66,eVar67,eVar68,eVar69,events";
s.linkTrackEvents = "event2,event6,event7,event8,event9,event10,event11,event12,event13,event14,event17,event18,event19,event26,event27,event29,event30,event31,event32,event33,event34,event35,event37,event39,event40,event41,event42,event43,event44,event45,event46,event47,event48,event49,event50,event51,event52,event53,event54,event55,event56,event57,event58,event59,event60,event61,event62,event63,event64,event65,event66,event67,event68,event69"

//s.linkTrackEvents="None"

// timeparting config variables
if (s.currentYear == undefined || s.currentYear == "") { // update s.currentYear to the current year
    var od = new Date();
    //Date d=Date.parse('3/7/' + od.getFullYear());
    var ys = od.getFullYear().toString();
    s.currentYear = ys;
}
if (s.dstStart == undefined || s.dstStart == "") {  // update s.dstStart to the correct Daylight Saving Time start date for the current year.
    var od=new Date();
    //Date d=Date.parse('3/7/' + od.getFullYear());
    var d = new Date(od.getFullYear(),2,7);
    //var n=d.getDay();

    var d1=new Date(od.getFullYear(),2,7);
    var dd= 0;
    while(dd<8){
        d1.setDate(d.getDate() + (++dd));
        //d.setDay(++dd)
        if (d1.getDay()==0) {
            var ds="";
            ds=(d1.getMonth() + 1) + "/" + d1.getDate() + "/" + d1.getFullYear();
            s.dstStart = ds;
            break;
        }
    }
...
if (s.dstEnd == undefined || s.dstEnd == "") {  // update s.dstEnd to the correct Daylight Saving Time end date for the current year.
    var od = new Date();
    //Date d=Date.parse('3/7/' + od.getFullYear());
    var d = new Date(od.getFullYear(), 10, 1);
    //var n=d.getDay();

    var d1 = new Date(od.getFullYear(), 10, 1);
    var dd = 0;
    var dd= 0;
    while(dd<7){
        d1.setDate(d.getDate() + (dd++));
...
function s_doPlugins(s) {
    ...
    /*timeparting*/
    s.prop23 = s.getTimeParting('d', '-5'); // Set weekday
    s.eVar36 = s.prop23;
    s.prop24 = s.getTimeParting('h', '-5'); // Set hour of day
    s.eVar37 = s.prop24;
    s.prop25 = s.getTimeParting('w', '-5'); // Set weekday v weekend
    s.eVar38 = s.prop25;
    /*timeparting end*/
    ...
    //Owners Entry Variables
    if (typeof gsc_prop1 != 'undefined')
        if (gsc_prop1 != '')
            s.prop1 = gsc_prop1;
    if (typeof gsc_prop28 != 'undefined')
        if (gsc_prop28 != '')
            s.prop28 = gsc_prop28;
    if (typeof gsc_evar20 != 'undefined')
        if (gsc_evar20 != '')
            s.eVar20 = gsc_evar20;
    if (typeof gsc_evar21 != 'undefined')
        if (gsc_evar21 != '')
            s.eVar21 = gsc_evar21;



    //Save PageName for Link Tracking (in case we mess up the pagenames in the future)
    if (s.pageName != "") {
        s.eVar34 = s.pageName;
        s.prop18 = s.pageName;
    }
    else if (s.pageURL != undefined && s.pageURL!="") {
        s.eVar34 = s.pageURL;
        s.prop18 = s.pageURL;
    }
    else if (s.wd.location.href != undefined && s.wd.location.href != "") {
        s.eVar34 = s.wd.location.href;
        s.prop18 = s.wd.location.href;
    }

    if (s.wd.location.href != undefined && s.wd.location.href != "") {
        s.eVar35 = s.wd.location.href;
    }

    var localEvents = "";
    if (!(!s.events)) {
        if (s.events != "") {
            localEvents = s.events;
            localEvents = localEvents.replace("event36,", "");
            localEvents = localEvents.replace(",event36", "");
            localEvents = localEvents.replace("event36", "");
            if (localEvents != s.events) {
                s.events = localEvents;
            }
        }
    }


    /* NEW Hierarchical s.Props */
    s.prop20 = "";
    s.prop21 = "";
    s.prop22 = "";
    s.prop26 = "";
    ...
    if (!(!s.prop3))
        if (s.prop3 != "")
            if (s.prop3 != "index")
                s.prop20 = prop2 + "\\" + s.prop3;

    if (!(!s.prop4))
        if (s.prop4 != "")
            if (s.prop4 != "index")
                s.prop21 = prop2 + "\\" + s.prop3 + "\\" + s.prop4;

    if (!(!s.prop5))
        if (s.prop5 != "")
            if (s.prop5 != "index")
                s.prop22 = prop2 + "\\" + s.prop3 + "\\" + s.prop4 + "\\" + s.prop5;

    if (!(!s.prop6))
        if (s.prop6 != "")
            if (s.prop6 != "index")
                s.prop26 = prop2 + "\\" + s.prop3 + "\\" + s.prop4 + "\\" + s.prop5 + "\\" + s.prop6;
    ...
    if (s.eVar6 == "") {
        ...
        var sTEVar6 = s.eVar6.toString();
        if (sTEVar6.indexOf("/", 0) == 0) sTEVar6 = sTEVar6.substr(1, sTEVar6.length - 1);
        s.eVar6 = sTEVar6;
        //sTEVar6 = sTEVar6.replace("/Product/", "");
        if (sTEVar6.indexOf("Product", 0) == 0) sTEVar6 = sTEVar6.substr(7, sTEVar6.length - 7);
        if (sTEVar6.indexOf("/", 0) == 0) sTEVar6 = sTEVar6.substr(1, sTEVar6.length - 1);
        ...
        var sTEVar43 = "";
        if (sTEVar6.length > 0) {
            var sMySplit = sTEVar6.split("/");
            for (i = 0; i < sMySplit.length; i++) {
                if (i == 0) sTEVar26 = sMySplit[i];
                else if (i == 1) sTEVar27 = sMySplit[i];
                else if (i == 2) sTEVar28 = sMySplit[i];
                else if (i == 3) sTEVar29 = sMySplit[i];
                else if (i == 4) sTEVar43 = sMySplit[i];
            }
        }
        s.eVar26 = sTEVar26;
        if (sTEVar27 != "") s.eVar27 = s.eVar26 + "\\" + sTEVar27;
        if (sTEVar28 != "") s.eVar28 = s.eVar27 + "\\" + sTEVar28;
        if (sTEVar29 != "") s.eVar29 = s.eVar28 + "\\" + sTEVar29;
        if (sTEVar43 != "") s.eVar43 = s.eVar29 + "\\" + sTEVar43;
    }
    ...
    /* CODE TO ACCEPT GENERIC Prop VALUES */
    if (typeof (sGenericPropValue) != "undefined") {
        sGenericPropValue = sGenericPropValue.replace(/[^\x00-\x7f]/g, '');
        switch (numGenericPropNumber) {
            case 1:
                s.prop1 = sGenericPropValue;
                break;
            case 2:
                s.prop2 = sGenericPropValue;
                break;
            case 3:
                s.prop3 = sGenericPropValue;
                break;
            case 4:
                s.prop4 = sGenericPropValue;
                break;
            case 5:
                s.prop5 = sGenericPropValue;
                break;
            case 6:
                s.prop6 = sGenericPropValue;
                break;
            case 7:
                s.prop7 = sGenericPropValue;
                break;
            case 8:
                s.prop8 = sGenericPropValue;
                break;
            case 9:
                s.prop9 = sGenericPropValue;
                break;
            case 10:
                s.prop10 = sGenericPropValue;
                break;
            case 11:
                s.prop11 = sGenericPropValue;
                break;
            case 12:
                s.prop12 = sGenericPropValue;
                break;
            case 13:
                s.prop13 = sGenericPropValue;
                break;
            case 14:
                s.prop14 = sGenericPropValue;
                break;
            case 15:
                s.prop15 = sGenericPropValue;
                break;
            case 16:
                s.prop16 = sGenericPropValue;
                break;
            case 17:
                s.prop17 = sGenericPropValue;
                break;
            case 18:
                s.prop18 = sGenericPropValue;
                break;
            case 19:
                s.prop19 = sGenericPropValue;
                break;
            case 20:
                s.prop20 = sGenericPropValue;
                break;
            case 21:
                s.prop21 = sGenericPropValue;
                break;
            case 22:
                s.prop22 = sGenericPropValue;
                break;
            case 23:
                s.prop23 = sGenericPropValue;
                break;
            case 24:
                s.prop24 = sGenericPropValue;
                break;
            case 25:
                s.prop25 = sGenericPropValue;
                break;
            case 26:
                s.prop26 = sGenericPropValue;
                break;
            case 27:
                s.prop27 = sGenericPropValue;
                break;
            case 28:
                s.prop28 = sGenericPropValue;
                break;
            case 29:
                s.prop29 = sGenericPropValue;
                break;
            case 30:
                s.prop30 = sGenericPropValue;
                break;
            case 31:
                s.prop31 = sGenericPropValue;
                break;
            case 32:
                s.prop32 = sGenericPropValue;
                break;
            case 33:
                s.prop33 = sGenericPropValue;
                break;
            case 34:
                s.prop34 = sGenericPropValue;
                break;
            case 35:
                s.prop35 = sGenericPropValue;
                break;
            case 36:
                s.prop36 = sGenericPropValue;
                break;
            case 37:
                s.prop37 = sGenericPropValue;
                break;
            case 38:
                s.prop38 = sGenericPropValue;
                break;
            case 39:
                s.prop39 = sGenericPropValue;
                break;
            case 40:
                s.prop40 = sGenericPropValue;
                break;
            default: break;
        }
    }
    ...
    /* CODE TO ACCEPT GENERIC EVAR VALUES FROM T and T BLOCKS */
    if (typeof (sGenericEVarValue) != "undefined") {
        switch (numGenericEVarNumber) {
            case 1:
                s.eVar1 = sGenericEVarValue;
                break;
            case 2:
                s.eVar2 = sGenericEVarValue;
                break;
            case 3:
                s.eVar3 = sGenericEVarValue;
                break;
            case 4:
                s.eVar4 = sGenericEVarValue;
                break;
            case 5:
                s.eVar5 = sGenericEVarValue;
                break;
            case 6:
                s.eVar6 = sGenericEVarValue;
                break;
            case 7:
                s.eVar7 = sGenericEVarValue;
                break;
            case 8:
                s.eVar8 = sGenericEVarValue;
                break;
            case 1:
                s.eVar9 = sGenericEVarValue;
                break;
            case 1:
                s.eVar9 = sGenericEVarValue;
                break;
            case 10:
                s.eVar10 = sGenericEVarValue;
                break;
            case 11:
                s.eVar11 = sGenericEVarValue;
                break;
            case 12:
                s.eVar12 = sGenericEVarValue;
                break;
            case 13:
                s.eVar13 = sGenericEVarValue;
                break;
            case 14:
                s.eVar14 = sGenericEVarValue;
                break;
            case 15:
                s.eVar15 = sGenericEVarValue;
                break;
            case 16:
                s.eVar16 = sGenericEVarValue;
                break;
            default: break;
        }
    }
    /* CODE TO ACCEPT GENERIC EVENTS */
    if (typeof (sGenericEvent) != "undefined") {
        if (sGenericEvent != "") {
            s.events = s.apl(s.events, sGenericEvent, ',', 1);
        }
    }

    /* NEWS ARTICLE ID */
    var sNewsID = s.getQueryParam('newsid');
    if (sNewsID > '')
        s.prop12 = 'News ID: ' + sNewsID;


    /* LOCAL OFFERS AND INVENTORY REGION ID */
    var sRegionID = s.getQueryParam('id');
    if (sRegionID == '')
        sRegionID = s.getQueryParam('regionid');
    var sThisPageName = s.pageName;

    if (sThisPageName.indexOf("find a home - Search") > 0) {
        sThisPageName = sThisPageName.replace("find a home - Search", "find a home - search");
        s.pageName = sThisPageName;
    }
    if (sRegionID == '') {
        if ((sThisPageName.indexOf("offers and events") > 0) || (sThisPageName.indexOf("find a home - inventoryhomes") > 0)) {
            var sbcType = s.read_cookie('bcType');
            if (sbcType == "region")
                sRegionID = s.read_cookie('bcId');
        }
    }
    if ((sRegionID != null) && (sRegionID != '')) {
        s.prop13 = sRegionID;
        if (sThisPageName.indexOf("offers and events") > 0)
            s.prop17 = sRegionID;
    }



    /* SOURCE CODE FOR EVENTS IPDETECTION */
    s.prop9 = s.getQueryParam('source');
    if (!s.prop9)
        s.prop9 = s.getQueryParam('Source');
    if (!s.prop9)
        s.prop9 = s.getQueryParam('SOURCE');

    if (s.prop9 == 'ipdetection')
        s.eVar13 = 'ipdetection';
    if (s.prop9 == 'modal')
        s.eVar13 = 'modal';

    if (s.prop9 == 'select')
        if (s.events.indexOf('event2') < 0) //Don't add to the SELECT events if this is just a lead submission
            s.events = s.apl(s.events, 'event8', ',', 1);
    //if (s.prop9 == 'select')
    //    s.eVar11 = 'select';


    //if (s.eVar6 == '' || (s.channel != '' && s.channel != s.eVar6))
    if (s.eVar6 == '')
            s.eVar6 = s.channel;
    ...
    /* LOCAL OFFERS COMMUNITY AND HOT HOMES EVARS */
    if (window.regionOffer_fName != null) {
        if (window.regionOffer_fName > '') {
            if (s.events.indexOf('event2') >= 0) {
                s.eVar14 = s.channel.split("/")[0];
                s.eVar15 = "Basic";
                s.eVar16 = s.eVar14;
                if (document.LandingSignUpForm.message.value != "")
                    if (document.LandingSignUpForm.message.value.length > 9)
                        if (document.LandingSignUpForm.message.value.substr(0, 9) == 'Hot Homes') {
                            s.eVar15 = 'Hot Homes';
                            s.eVar16 = "";
                            s.eVar17 = s.eVar14;
                        }
    ...
    /* LOCAL OFFERS COMMUNITY AND HOT HOMES GET MAP EVARS */
    if ((s.eVar16 == "") && (s.eVar17 == "") && (s.eVar6 != "")) {
        if ((s.events.indexOf('event13') >= 0) || (s.events.indexOf('event14') >= 0)) {
            s.eVar14 = s.eVar6;
        }
    }


    /* LOCAL OFFERS COMMUNITY AND HOT HOMES GET MAP EVENTS */
    /* ADD EVENT41 AND SET EVAR1 TO 'LOCAL OFFERS' FOR SEGMENT REDUCTION OF COMMUNITY DRIVING EXPORT */
    if ((s.events.indexOf('event13') >= 0) || (s.events.indexOf('event14') >= 0)) {
        s.eVar1 = 'Local Offers';
        s.events = s.apl(s.events, 'event41', ',', 1)  //,1 = only add if item not already in list (case insensitive)
    }


    /* CODE TO SET EVAR19 - CONTACT FORM OPTION */
    if (typeof (contactFormOption) != "undefined") {
        s.eVar19 = contactFormOption;
    }


    /* CODE TO SET PROP7 - TRACK PARAMETER */
    s.prop7 = '';
    s.prop7 = s.getQueryParam('track');
    if (!s.prop7)
        s.prop7 = s.getQueryParam('Track');
    if (!s.prop7)
        s.prop7 = s.getQueryParam('TRACK');
    ...
    s.prop11 = s.eVar11 = s.getVisitNumCustom('m'); // monthly visits
    ...
    /* GenericLinkClick Tracking */
    var strGenericLink = "";
    if (strGenericLink == "") {
        strGenericLink = s.getQueryParam('buildID');  //only buildID currently
        if (strGenericLink != "") {
            strGenericLink = "BuildID=" + strGenericLink;
        }
    }
    ////TEMPLATE FOR ADDITIONALS
    //if (strGenericLink == "") {
    //    strGenericLink = s.getQueryParam('strGenericLinkParam');
    //    if (strGenericLink != "") {
    //        strGenericLink = "ParamName=" + strGenericLink;
    //    }
    //}

    if (strGenericLink != "") {
        s.eVar33 = strGenericLink;
    ...
    /* has option bundles */
    s.prop27 = "";
    if (typeof (hasBundles) != "undefined") {
        if (hasBundles == "true") {
            s.prop27 = "HasOptionBundles";
        }
    }

    /* has life tested */
    s.prop29 = "";
    if (typeof (hasLifeTested) != "undefined") {
        if (hasLifeTested.toString() == "true") {
            s.prop29 = "HasLifeTested";
        }
    }

    /* Strip Extended Characters from eVar21 (for Promo Names) */
    if (typeof (s.eVar21) != "undefined") {
        s.eVar21 = s.eVar21.replace(/[^\x00-\x7f]/g, '');
    }
    /* and eVar22 also */
    if (typeof (s.eVar22) != "undefined") {
        s.eVar22 = s.eVar22.replace(/[^\x00-\x7f]/g, '');
    }
...
/*
*Plugin:  read_cookie custom plugin v.1.0 - read cookie value - mcgill
*/
s.read_cookie = new Function("c_name", ""
...
s.visitorNamespace = "pultehomes";
s.trackingServer = sLocalTrackingServer;
//s.trackingServer="pultehomes.122.2o7.net"
